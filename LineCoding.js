...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const midY = height / 2;
  let currentAnim = 0;
  let currentUnitWidth = 50;
  let binaryDataLength = 1;
  ...
  const techniqueInfo = {
    ...
    "rz": {
      title: "Return to Zero (RZ) Encoding",
      description: "Uses a mid-bit return to zero voltage level for representing bits, ensuring better clock synchronization."
    },
    "nrzl": {
  ...
  // Return to Zero (RZ)
  function drawRZ(binaryData, index, callback, animId) {
    const leftMargin = 30;
    const unitWidth = currentUnitWidth;
    let x = leftMargin + index * unitWidth;
    const high = midY - 50;
    ...
    let y = binaryData[index] === "1" ? high : low;
    drawLine(x, y, x + unitWidth / 2, y, () => {
      drawLine(x + unitWidth / 2, y, x + unitWidth / 2, midY, () => {
        drawLine(x + unitWidth / 2, midY, x + unitWidth, midY, callback, animId);
      }, animId);
    }, animId);
  }
  ...
  function drawManchester(binaryData, index, callback, animId) {
    ...
    if (binaryData[index] === "0") {
      // For a '0': first half at high level, then vertical drop to low.
      ...
      // For a '1': first half at low level, then vertical rise to high.
